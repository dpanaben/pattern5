...
sanitize4 = Sanitize.new(:match => '\s\!|\!|\!\s', :result => '！', :description => '把 ! 改成全形字')
sanitize5 = Sanitize.new(:match => '\s\%|\%|\%\s', :result => '％', :description => '把 % 改成全形字')
sanitize6 = Sanitize.new(:match => '\s\:|\:|\:\s', :result => '：', :description => '把 : 改成全形字')
sanitize7 = Sanitize.new(:match => '\s\;|\;|\;\s', :result => '；', :description => '把 ; 改成全形字')
sanitize8 = Sanitize.new(:match => '\s\~|\~|\~\s', :result => '～', :description => '把 ~ 改成全形字')
sanitize9 = Sanitize.new(:match => '\s\?|\?|\?\s', :result => '？', :description => '把 ? 改成全形字')
...
post1 = Post.new(user_id: 1, :title => 'FortiWeb-1000C與3000C', :content => '
 
...
 
')
post2 = Post.new(user_id: 1, :title => '這是用來測試. ', :content => '這是用來測試.
...
post3 = Post.new(user_id: 1, :title => '引號很難搞', :content => '引號很難搞，因為如果是巢狀引號，很難判別究竟是誰包著誰。
...
應該變成：
「更厲害的來了，還有全形的喔！而且兩個不一樣，前面是『上引號』，後一個是『下引號』。」')
post4 = Post.new(user_id: 1, :title => ', ( ) ! / % : ; ~ ?', :content => ', ( ) ! / % : ; ~ ?
, ( ) ! / % : ; ~ ?
, ( ) ! / % : ; ~ ?
, ( ) ! / % : ; ~ ?
, ( ) ! / % : ; ~ ?
